...
- If audio/screenshot capture fails (e.g. tab audio permission revoked), the card can still be saved as text-only (Word/Sentence kinds); Audio Word/Audio Sentence kinds are disabled in the selector until their asset resolves, with an inline retry action rather than a silent placeholder.
